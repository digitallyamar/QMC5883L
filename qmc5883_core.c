 ...
 */
 ...
 /* Scale in nT/LSB: ±2G = ~8333 nT/LSB, ±8G = ~33333 nT/LSB */
 static const int qmc5883_regval_to_nanoscale[] = { 
	 8333, 33333 
 };
 ...
 static int qmc5883_get_oversampling_index(struct qmc5883_data *data, int val, int val2)
 {
     int i;
     for (i = 0; i < data->variant->n_regval_to_oversampling_ratio; i++)
	 if (val == data->variant->regval_to_oversampling_ratio[i][0] &&
	     val2 == data->variant->regval_to_oversampling_ratio[i][1])
	     return i;
     return -EINVAL;
 }
				 ...
				 struct device_attribute *attr, char *buf)
 {
	 ...
	 for (i = 0; i < data->variant->n_regval_to_oversampling_ratio; i++) {
		 pr_info("Loading to buf=%d\n", data->variant->regval_to_oversampling_ratio[i][0]);
 
		 len += scnprintf(buf + len, PAGE_SIZE - len,
		 "%d ", data->variant->regval_to_oversampling_ratio[i][0]);
	 }
 ...
 static IIO_DEVICE_ATTR(in_magn_oversampling_ratio_available, S_IRUGO,
				 ...
				 struct device_attribute *attr, char *buf)
 {
	 ...
	 for (i = 0; i < data->variant->n_regval_to_full_scale; i++) {
		 pr_info("Loading to buf=0.%09d\n", data->variant->regval_to_nanoscale[i]);
			 ...
			 int val, int val2, long mask)
 {
	 ...
	 switch (mask) {
		 ...
		 case IIO_CHAN_INFO_OVERSAMPLING_RATIO:
			 rate = qmc5883_get_oversampling_index(data, val, val2);
			 if (rate < 0) {
			     pr_info("Amar: get_oversampling rate error\n");
			     return -EINVAL;
			 }
			 return qmc5883_set_oversampling_ratio(data, rate);
 ...
 static struct attribute *qmc5883_attributes[] = {
	 ...
	 &iio_dev_attr_in_magn_oversampling_ratio_available.dev_attr.attr,
 ...
 static const struct qmc5883_chip_info qmc5883_chip_info_tbl[] = {
	 [QMC5883_ID] = {
		 ...
		 .regval_to_nanoscale = qmc5883_regval_to_nanoscale,
		 .n_regval_to_full_scale = ARRAY_SIZE(qmc5883_regval_to_nanoscale),
	 }
 };
 ...
 static int qmc5883_init(struct qmc5883_data *data)
 {
	 ...
	 ret = regmap_update_bits(data->regmap, QMC5883_CONTROL_REG_1,
		 QMC5883_RANGE_GAIN_MASK,
		 QMC5883_RANGE_GAIN_DEFAULT << QMC5883_RANGE_GAIN_OFFSET);
	 if (ret < 0) {
